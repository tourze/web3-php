...
use Tourze\Web3PHP\Methods\Eth\GetBlockTransactionCountByHash;
...
/**
 * GetBlockTransactionCountByHash 测试
 * @internal
 */
#[CoversClass(GetBlockTransactionCountByHash::class)]
final class GetBlockTransactionCountByHashTest extends TestCase
{
    /**
     * 测试构造函数
     */
    ...
    {
        $method = new GetBlockTransactionCountByHash();

        $this->assertInstanceOf(GetBlockTransactionCountByHash::class, $method);
        $this->assertSame('eth__get_block_transaction_count_by_hash', (string) $method);
    }
    ...
    public function testDefaultProperties(): void
    {
        ...
        $this->assertSame([BlockHashValidator::class], $validators->getValue($method));

        // 检查inputFormatters属性
        $inputFormatters = $reflection->getProperty('inputFormatters');
        $inputFormatters->setAccessible(true);
        $this->assertSame([HexFormatter::class], $inputFormatters->getValue($method));
    ...
    public function testToPayload(): void
    {
        ...
        $this->assertSame('eth__get_block_transaction_count_by_hash', $payload['method']);
    ...
    public function testToPayloadString(): void
    {
        ...
        $this->assertStringContainsString('eth__get_block_transaction_count_by_hash', $payloadString);
